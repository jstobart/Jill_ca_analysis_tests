...
Settings.MainDir = 'E:\Data\Two_Photon_Data\P2Y1_Mice';
...
Settings.ScoreSheetNames = {
        'PYRG04_Scoresheet_LongTrials.xls',...
        'PYRG02_Scoresheet_LongTrials.xls',...
    };
...
SaveFiles{1,1} = fullfile(Settings.MainDir, 'Results', 'P2Y_cGC&RC_PYRG04.csv');%'Control_Peaks_3Conds.csv'); % all data
SaveFiles{1,2} = fullfile(Settings.MainDir, 'Results', 'P2Y_cGC&RC_PYRG04.mat');%'Control_Peaks_3Conds.csv'); % all data
SaveFiles{1,3}= fullfile(Settings.MainDir, 'Results','P2Y_cGC&RC_traces_PYRG04.mat'); %'Control_TraceAUC_20sWindow_3Conds.csv'); % neuronal hand click cell scan
...
if ~exist(fullfile(Settings.MainDir, 'Results','RCaMP_cGCaMP_Matrix.mat'),'file')
    ...
    unmixFile = 'E:\Data\Two_Photon_Data\P2Y1_Mice\PYRG04\2017_4_25\spot1_long_Nostim\highres_spot1_long016.tif';
...
for iAnimal = 1:numAnimals
    ...
    for iSpot = 1:length(spots) % looks at every 2nd line of SpotIDs-
        ...
        for iCond = 1:length(Settings.NameConditions)
            ...
            %cell was dark
            %ImgArray.exclude_frames('badframes',(1:5),'method', 'inpaint','inpaintIters',5);
            
            
            % Spectral Unmixing of GCaMP and RCaMP
            ImgArray= ImgArray.unmix_chs(false,[],RCaMP_cGCaMP_Matrix{1,1});
