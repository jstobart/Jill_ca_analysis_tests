...
SaveFiles{1,1} = fullfile(Settings.MainDir, 'Results', 'P2Y_cGC&RC_28_4_2017.csv');%'Control_Peaks_3Conds.csv'); % all data
SaveFiles{1,2} = fullfile(Settings.MainDir, 'Results', 'P2Y_cGC&RC_28_4_2017.mat');%'Control_Peaks_3Conds.csv'); % all data
SaveFiles{1,3}= fullfile(Settings.MainDir, 'Results','P2Y_cGC&RC_traces_28_4_2017.mat'); %'Control_TraceAUC_20sWindow_3Conds.csv'); % neuronal hand click cell scan
...
for iAnimal = 1:numAnimals
    ...
    for iSpot = 1:length(spots) % looks at every 2nd line of SpotIDs-
        ...
        for iCond = 1:length(Settings.NameConditions)
            ...
            ImgArray= ImgArray.unmix_chs(false, [], cell2mat(RCaMP_cGCaMP_Matrix));
