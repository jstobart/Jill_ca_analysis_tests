...
SaveFiles{1,1} = fullfile(Settings.MainDir, 'Results', 'fracActive_test_11_2017.csv');%'Control_Peaks_3Conds.csv'); % all data
SaveFiles{1,2} = fullfile(Settings.MainDir, 'Results', 'fracActive_test_11_2017.mat');%'Control_Peaks_3Conds.csv'); % all data
SaveFiles{1,3}= fullfile(Settings.MainDir, 'Results','fracActive_test_11_2017.mat'); %'Control_TraceAUC_20sWindow_3Conds.csv'); % neuronal hand click cell scan
...
for iAnimal = 1:numAnimals
    ...
    if Settings.PMTnum(1,1)==2
        load(fullfile(Settings.MainDir, 'Results','RCaMP_mGCaMP_Matrix_Ch2.mat'));
    elseif Settings.PMTnum(1,1)==2
        load(fullfile(Settings.MainDir, 'Results','RCaMP_mGCaMP_Matrix_Ch4.mat'));
    end
                ...
                for itrial=1:size(CellScans,2)
                    ...
                    for iPeak = 1:length(temp.amplitude)
                        ...
                        if iScan==1 || iScan==3
                            ...
                            ROIpuffIdx = CellScans(iScan,itrial).calcFindROIs.data.roiIdxs(ROIindex);
                            x_pix= Settings.Xres(1,1); y_pix= Settings.Yres(1,1);
                            ...
                            for kROI= 1:length(CellScans(compScan,1).calcFindROIs.data.roiNames)
                                ...
                                ROIMask{kROI}= CellScans(compScan,1).calcFindROIs.data.roiMask(:,:,kROI);
                    ...
                    if strcmp(CellScans(iScan,itrial).calcFindROIs.data.roiNames{1,1}, 'none')
                        ...
                        for iROI = 1:size(traces,2)
                            ...
                            if iScan==1 || iScan==3
                                Trace_data{iROI,9} = CellScans(iScan,itrial).calcFindROIs.data.centroid{iROI,1};
                                Trace_data{iROI,10} = CellScans(iScan,itrial).calcFindROIs.data.puffIdxs{iROI,1};
                                Trace_data{iROI,11} = CellScans(iScan,itrial).rawImg.metadata.pixelSize;
                            else
                                
                                
                                Trace_data{iROI,9} = 0;
                                Trace_data{iROI,10} = CellScans(iScan,itrial).calcFindROIs.data.roiMask(:,:,iROI);
                                Trace_data{iROI,11} = CellScans(iScan,itrial).rawImg.metadata.pixelSize;
                            end
                            
                            % get the indices  and area for a particular ROI
                            if iScan==1 || iScan==3
                                ROIpuffIdx = CellScans(iScan,itrial).calcFindROIs.data.roiIdxs{iROI,1};
                                x_pix= Settings.Xres(1,1); y_pix= Settings.Yres(1,1);
                                [jy,jx,~] = ind2sub([y_pix,x_pix],cell2mat(ROIpuffIdx));
                                jx = round(mean(jx));
                                jy = round(mean(jy));
                                ...
                                if exist('spatialcorr','var')
                                    indx = find(spatialcorr>0);
                                    Trace_data{iROI,12}= CellScans(compScan,1).calcFindROIs.data.roiNames{indx};
                                else
