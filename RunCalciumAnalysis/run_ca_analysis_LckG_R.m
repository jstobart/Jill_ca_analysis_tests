...
SaveFiles{1,1} = fullfile(Settings.MainDir, 'Results', 'Peaks_allMice_Lck_nostim_vs_longstim_12_2017.csv');%'Control_Peaks_3Conds.csv'); % all data
SaveFiles{1,2} = fullfile(Settings.MainDir, 'Results', 'Peaks_allMice_Lck_nostim_vs_longstim_12_2017.mat');%'Control_Peaks_3Conds.csv'); % all data
SaveFiles{1,3}= fullfile(Settings.MainDir, 'Results','Traces_allMice_Lck_nostim_vs_longstim_12_2017.mat'); %'Control_TraceAUC_20sWindow_3Conds.csv'); % neuronal hand click cell scan
                ...
                for itrial=1:size(CellScans,2)
                    ...
                    if strcmp(CellScans(iScan,itrial).calcFindROIs.data.roiNames{1,1}, 'none')
                        ...
                        for iROI = 1:size(traces,2)
                            ...
                            Trace_data{iROI,13} = CellScans(1, 1).rawImg.metadata.frameRate; % frameRate
                        end
