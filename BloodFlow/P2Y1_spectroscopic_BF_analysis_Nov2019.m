...
Settings.ResultsFolder = 'G:\ZurichData\LaserSpeckle_MWSpec_Data\MWSpec\Results';
...
SaveFiles{1,1} = fullfile(Settings.ResultsFolder,'FilesforMatlab', 'P2YRG7_MWSpecData.mat');
SaveFiles{1,2} = fullfile(Settings.ResultsFolder,'FilesforR','P2YRG7_MWSpecData.csv');
...
Settings.FileNames = {  % folder names where images and roiSet.zip is found
    ...
    'G:\ZurichData\LaserSpeckle_MWSpec_Data\MWSpec\P2YRG7\baseline_25102017\Trial_1',...
    'G:\ZurichData\LaserSpeckle_MWSpec_Data\MWSpec\P2YRG7\baseline_25102017\Trial_2',...
    };

Settings.Baseline = 4; % time (s) before the whisker stimulator starts,  2s for short trials (10s long), 5s for long trials
Settings.Animal = 'P2YRG7';
Settings.Timepoint= 'baseline';

channel = struct('intrinsic_optical',6);%,'intrinsic_optical',2,'intrinsic_optical',3,'intrinsic_optical',4,'intrinsic_optical',5,'intrinsic_optical',6);  % what's on each channel
...
for Iteration= 1:length(Settings.FileNames)
    ...
    % find dat file for each trial
    for iTrial= 1:length(dataPath)
        DatName=dataPath(iTrial);
        
        % Create Spectroscopic Object from dat file
        SpectroscopicImageObj = SpectroscopicImage(DatName, channel, CalibrationFile);
        
        % load data for analysis
        BloodFlow = SpectroscopicOI(DatName,SpectroscopicImageObj,[]); % peaks from automated ROIs
        ...
        HbO = (BloodFlow.calcSOI.data.c_hbo-BloodFlow.calcSOI.config.co_hbo)./...
            BloodFlow.calcSOI.config.co_hbo;
        HbR = (BloodFlow.calcSOI.data.c_hbr-BloodFlow.calcSOI.config.co_hbr)./...
            BloodFlow.calcSOI.config.co_hbr;
        HbT = (BloodFlow.calcSOI.data.c_hbt-BloodFlow.calcSOI.config.co_hbt)./...
            BloodFlow.calcSOI.config.co_hbt;
        
        plot_hbo = squeeze(mean(mean(HbO(:,:,:),1),2));
        plot_hbo = plot_hbo(2:32);
        plot_hbr = squeeze(mean(mean(HbR(:,:,:),1),2));
        plot_hbr = plot_hbr(2:32);
        plot_hbt = squeeze(mean(mean(HbT(:,:,:),1),2));
        plot_hbt = plot_hbt(2:32);

        % HBO, HBR, HBT traces to plot later
        
        Traces{iTrial,1} =  Settings.Animal;
        Traces{iTrial,1} =  Settings.Timepoint;
        Traces{iTrial,1} =  Settings.Baseline;
