...
SaveFiles{1,1} = fullfile(Settings.MainDir, 'Results', 'LckGC&RC_2p5D_23_02_2017.csv');%'Control_Peaks_3Conds.csv'); % all data
SaveFiles{1,2} = fullfile(Settings.MainDir, 'Results', 'LckGC&RC_2p5D_23_02_2017.mat');%'Control_Peaks_3Conds.csv'); % all data
SaveFiles{1,3}= fullfile(Settings.MainDir, 'Results','LckGC&RC_traces_2p5D_23_02_2017.mat'); %'Control_TraceAUC_20sWindow_3Conds.csv'); % neuronal hand click cell scan
    ...
    for iSpot = 1:length(spots) % looks at every 2nd line of SpotIDs-
        ...
        for iCond = 1:length(Settings.NameConditions)
            ...
            testRoot =SpotPaths{PathIdx};
                ...
                % FLIKA selected
                    findConf{4} = ConfigFindROIsFLIKA_2p5D.from_preset('ca_neuron', 'baselineFrames',...
                    BL_frames,'freqPassBand',1,'sigmaXY', 2,...
          ...
          %% Output data
            ...
            for itrial=1:length(CSArray_Ch2_FLIKA)
                ...
                for iROI = 1:size(traces,2)
                    ...
                    Trace_data{iROI,8} = traces(:,iROI);
                    Trace_data{iROI,9} = 0;
                    Trace_data{iROI,10} = CSArray_Ch2_FLIKA(1,itrial).calcFindROIs.data.roiMask(:,:,iROI);
                    Trace_data{iROI,12} = 0;
...
% doplot=0;
% 
% FileSave{1,1}='E:\Data\Two_Photon_Data\GCaMP_RCaMP\Lck_GCaMP6f\Results\LongStim_Correlations_23_02_2017.mat';
% FileSave{1,2}='E:\Data\Two_Photon_Data\GCaMP_RCaMP\Lck_GCaMP6f\Results\LongStim_Correlations_23_02_2017.csv';
% 
% % load data traces
