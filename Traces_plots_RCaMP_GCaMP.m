...
for iTrial=1:size(uniqueTrials,1)
    ...
    for xxTrial= 1:size(data_traces,1)
        Trial_str2(xxTrial)= strcmp(data_traces{xxTrial, 17},uniqueTrials{iTrial});
    end
    trialTraces=data_traces(Trial_str2,:);
    
    %find neuron and neuropil ROIs
    for iROI=1:size(trialData,1)
        Nstr{iROI}= strfind(trialData{iROI,23},'N*');
    end
    ...
    for iN= 1:size(NeuronData, 1)   
        ...
        for iTrace=1:size(trialTraces,1)
            ...
            ShiftTrace{iA,6}=trialTraces{iTrace,12};
            ShiftTrace{iA,7}=trialTraces{iTrace,1};
            ShiftTrace{iA,8}=trialTraces{iTrace,8};
            ShiftTrace{iA,9}=TimeX-(5+NeuronData{iN,6});
            ShiftTrace{iA,9}=TimeX-(5+NeuronData{iN,8});
        end
...
%% ROITypes Shifted Traces
figure ('name', 'Shifted traces- Neuron peak time')
subplot(1,5,1)
hold on
axis off
for xROI= 1:size(Resp_neuron_traces,2)
