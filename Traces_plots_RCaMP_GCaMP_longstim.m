...
%unique ROIs
ROIunique=unique(data_traces(:,14));

for xROI=1:size(ROIunique,1)
    CurrentROI=ROIunique(xROI,1);
    for iROI= 1:size(data_traces,1)
    ROIuniqueIdx(iROI)=(data_traces(iROI,14),CurrentROI)
end
...
for iROI=1:size(RespondingROIs,1)
    ...
    for xROI= 1:length(earlyROIs)
        Early_Str= strfind(RespondingROIs{iROI, 14},earlyROIs{xROI,1});
        
        if ~isempty(Early_Str)
