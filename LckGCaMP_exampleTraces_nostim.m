...
for xROI=1:size(Nostim,1)
    GCNP2(xROI)=(strcmp(Nostim{xROI,3},'GCaMP')&& strcmp(Nostim{xROI,1},'np'));
end
...
for iROI=1:length(Nostim)
    ...
    D_str2= strfind(Nostim{iROI, 1},'r') && strcmp(Nostim{iROI,3},'RCaMP'); %automated dendrite
    P_str= strfind(Nostim{iROI, 1},'r') && strcmp(Nostim{iROI,3},'GCaMP');
...
% get rid of astrocyte neuropil traces
for xROI=1:size(NostimPeaks,1)
    GCNP2(xROI)=(strcmp(NostimPeaks{xROI,14},'GCaMP')&& strcmp(NostimPeaks{xROI,10},'np'));
end

NostimPeaks=NostimPeaks(~GCNP2',:);


for iROI=1:length(NostimPeaks)
    %find ROITypes
    N_str= strfind(NostimPeaks{iROI, 10},'N');
    D_str= strfind(NostimPeaks{iROI, 10},'D'); %hand selected dendrite
    D_str2= strfind(NostimPeaks{iROI, 10},'r') && strcmp(NostimPeaks{iROI,14},'RCaMP'); %automated dendrite
    P_str= strfind(NostimPeaks{iROI, 10},'r') && strcmp(NostimPeaks{iROI,14},'GCaMP');
    EF_str= strfind(NostimPeaks{iROI, 10},'E');
    ...
    % unique ROI names
    NostimPeaks{iROI,22}=strcat(NostimPeaks{iROI,13},'_',NostimPeaks{iROI,15},'_',NostimPeaks{iROI,12}, '_',NostimPeaks{iROI,10});
end
...
for itrial=1:length(Trials)
    ...
    AstropT = TrialData(~NeuronalIdx,5);
