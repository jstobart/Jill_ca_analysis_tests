...
high$responders="high"
mid$responders="mid"
low$responders="low"
Responders<-rbind(high,mid,low)


# merge responders into other data table
ROIdata<-merge(sum.data.raw.roi, Responders[, c("ROInameU", "responders")], by="ROInameU", all.x=TRUE)
...
ggplot(ROIdata, aes(x = treatment, y = ROInameU, fill = Amp_mean2)) +
...
# ampAfter-ampBefore/ampAfter+ampBefore

#calculate the Tamoxifen Index
diff.data.short.amp$TI.stim<-(diff.data.short.amp$TAM_Stim-diff.data.short.amp$None_Stim)/(diff.data.short.amp$TAM_Stim+diff.data.short.amp$None_Stim)
...
ggplot(diff.data.short.amp, aes(x = responders, y = TI.stim, fill=responders)) + 
  geom_boxplot() + 
  facet_grid(~area) + 
  theme_bw()+
  ggtitle("boxplot tamoxifen index stim")


ggplot(diff.data.short.amp, aes(x = TI.stim, fill=responders)) + 
  geom_density(alpha=0.2)+
  facet_grid(~area) + 
  theme_bw()+
  ggtitle("boxplot tamoxifen index")

ggplot(diff.data.short.amp, aes(x = TI.stim)) + 
  geom_density(alpha=0.2)+
  facet_grid(~area) + 
  theme_bw()+
  ggtitle("boxplot tamoxifen index")
