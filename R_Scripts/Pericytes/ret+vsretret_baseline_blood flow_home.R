...
ggplot(baseline, aes(x = BranchOrder, y = Depth, fill = Genotype)) + 
  geom_boxplot() + 
  ylab("depth [um]") + 
  scale_fill_manual(
    values=c("black", "red"), 
...
ggplot(baseline, aes(x = BranchGroup, y = Depth, fill = Genotype)) + 
  geom_boxplot() + 
  ylab("depth [um]") + 
  scale_fill_manual(
    values=c("black", "red"), 
...
depth.null = lmer(Depth ~ (1|AnimalName) + (1|Branchname), baseline,REML=FALSE)
depth.model1 = lmer(Depth~ BranchGroup + (1|AnimalName) + (1|Branchname), baseline,REML=FALSE)
depth.model2 = lmer(Depth~ Genotype + (1|AnimalName) + (1|Branchname), baseline,REML=FALSE)
depth.model3 = lmer(Depth~ Genotype + BranchGroup + (1|AnimalName) + (1|Branchname), baseline,REML=FALSE)
depth.model4 = lmer(Depth~ Genotype * BranchGroup+ (1|AnimalName) + (1|Branchname), baseline,REML=FALSE)
