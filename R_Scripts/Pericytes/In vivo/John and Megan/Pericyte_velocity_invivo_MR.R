...
df1B <- summarySE(AllData, measurevar="velocity", groupvars=c("Drug","Animal"))
...
#velocity
velocity.null = lmer(velocity ~ (1|Animal) + (1|SpotName) + (1|Vessel), AllData,REML=FALSE)
velocity.model1 = lmer(velocity~ Drug + (1|Animal) + (1|SpotName) + (1|Vessel), AllData,REML=FALSE)
velocity.anova <- anova(velocity.null, velocity.model1)
print(velocity.anova)

# p values
vel.drug <- lsmeans(velocity.model1, pairwise ~ Drug, glhargs=list())
summary(vel.drug)
