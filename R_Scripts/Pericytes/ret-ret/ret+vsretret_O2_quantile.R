...
#histogram
ggplot(allData, aes(x=pO2, y=..density..,fill=Genotype)) +
  geom_histogram(binwidth=4, position="dodge")+
  scale_fill_manual(
    values=c("blue", "red"))+
  max.theme

# ks test- compare distributions of pO2 for each genotype
pO2.kstest<- ks.test(allData$pO2[allData$Genotype=="Ret+"],allData$pO2[allData$Genotype=="RetRet"])
print(pO2.kstest)


##############

# classify as high and low pO2 based on the quantile of the control animals!
quantile_pO2=quantile(allData$pO2[allData$Genotype=="Ret+"], probs = seq(0, 1, 0.25))

allData$O2_type[allData$pO2<quantile_pO2[2]]<-"low"
allData$O2_type[allData$pO2>=quantile_pO2[4]]<-"high"

#depth of high and low??


# mean pO2

df1A<- summarySE(allData, measurevar="pO2", groupvars=c("Genotype", "O2_type"))
df1B<- summarySE(allData, measurevar="pO2", groupvars=c("Genotype", "O2_type","BranchOrder"))
df1C<- summarySE(allData, measurevar="pO2", groupvars=c("Genotype", "O2_type","AdjustedDepth"))


ggplot(data=df1A, aes(x=O2_type, y=pO2, fill=Genotype)) +
...
ggplot(data=df1B, aes(x=interaction(BranchOrder,O2_type), y=pO2, fill=Genotype)) +
  geom_bar(stat="identity", position=position_dodge(), colour="black") +
  geom_errorbar(aes(ymin=pO2-se, ymax=pO2+se), colour="black", width=.1,  position=position_dodge(.9)) +
  ylab("pO2 [mmHg]") +
  scale_fill_manual(
...
# define as arteries or veins based on the zero order vessels only
zeroOrder<-subset(allData, BranchOrder==0)
