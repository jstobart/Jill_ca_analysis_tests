...
Spot.ntrials<-ddply(all.peaks, c("Animal","Type","Spot","Condition"), summarise, nTrials=length(unique(Trial)))
...
all.peaks.window<-subset(all.peaks, peakTime>-0.9 & peakTime<12 & amplitude>0 & Duration<15)
...
pT.RC.S.model1 = lmer(peakTime ~ Type + (1|Animal) + (1|Spot) + (1|ROIs_trial), stim.peaks.RC,REML=FALSE)
...
ROInum.8strial.group<-ddply(stim.peaks.GC, c("Animal","Spot","Type","Condition","Channel","Group"), summarise, nROIs=length(unique(ROIs_trial_Cond)))
